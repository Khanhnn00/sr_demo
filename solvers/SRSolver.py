...
class SRSolver(BaseSolver):
    ...
    def _overlap_crop_forward(self, x, min_size=100000, bic=None):
        """
    ...
    def load(self):
        ...
        if (self.is_train and self.opt['solver']['pretrain']) or not self.is_train:
            ...
            print('===> Loading srfbn from [%s]...' % model_path)
            if self.is_train:
                ...
                print("======================"if isinstance(self.model, nn.DataParallel) else "nope")
                checkpoint = torch.load(model_path)
