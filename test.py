...
def main(model):
    ...
    opt_srfbn = option.parse('options/test/test_SRFBN.json')
    opt_edsr = option.parse('options/test/test_EDSR.json')
    if model == 'SRFBN':
        opt = option.dict_to_nonedict(opt_srfbn)
    else:
        opt = option.dict_to_nonedict(opt_edsr)
    ...
    for bm, test_loader in zip(bm_names, test_loaders):
        ...
        for img, name in zip(sr_list, path_list):
            cv2.imwrite(os.path.join(save_img_path, 'output.png'), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
            del img
